...
Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client, for example 'sedge generate full-node --execution nethermind:docker.image'. If you want to use the default docker image, just use the client name`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.distributed {
				services = append(services, distributedValidator)
			}
...
This command does not generate a validator configuration, as Optimism and Base use different validation mechanisms compared to standard Ethereum networks.`,
	...
	cmd.Flags().StringVar(&flags.l2ExecutionName, "op-execution", "", "Optimism Execution client to be used, op-nethermind, op-geth, or op-reth. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name.")
	cmd.Flags().StringVarP(&flags.executionName, "execution", "e", "", "Execution engine client, e.g. geth, nethermind, besu, erigon. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name")
	...
	cmd.Flags().StringVar(&flags.jwtPath, "jwt-secret-path", "", "Path to the JWT secret file")
	cmd.Flags().BoolVar(&flags.mapAllPorts, "map-all", false, "Map all clients ports to host. Use with care. Useful to allow remote access to the clients")
	cmd.Flags().StringVar(&flags.chain, "chain", "op", "You can set any name of the superchains supported by Optimism. Default is 'op' for Optimism. You can use `base` for Base, `world` for Worldchain, and subsequently.")
	cmd.Flags().StringSliceVar(&flags.fallbackEL, "fallback-execution-urls", []string{}, "Fallback/backup execution endpoints for the consensus client. Not supported by Teku. Example: 'sedge generate full-node -r --fallback-execution=https://mainnet.infura.io/v3/YOUR-PROJECT-ID,https://eth-mainnet.alchemyapi.io/v2/YOUR-PROJECT-ID'")
	cmd.Flags().StringArrayVar(&flags.elExtraFlags, "el-extra-flag", []string{}, "Additional flag to configure the execution client service in the generated docker-compose script. Example: 'sedge generate full-node --el-extra-flag \"<flag1>=value1\" --el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.ell2ExtraFlags, "el-op-extra-flag", []string{}, "Additional flag to configure the execution client for optimism service in the generated docker-compose script. Example: 'sedge generate full-node --el-extra-flag \"<flag1>=value1\" --el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
...
func TaikoFullNodeSubCmd(sedgeAction actions.SedgeActions) *cobra.Command {
	var flags GenCmdFlags
	cmd := &cobra.Command{
		Use:   "taiko-full-node [flags]",
		Short: "Generate a full node config for Taiko",
		Long: `Generate a docker-compose and an environment file with a full node configuration for Taiko networks.

This command sets up a Taiko full node, which includes an execution client, a consensus client, a Taiko driver client, and an Taiko Execution node.

If you don't provide images for your clients, they will be chosen randomly. You can specify custom images for the Taiko Driver and other nodes.

The command allows you to use external execution and consensus APIs instead of running your own nodes, by providing the respective URLs.

Additionally, you can use the syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client, for example 'sedge generate taiko-full-node --execution nethermind:custom.image'. If you want to use the default docker image, just use the client name.

This command does not generate a validator configuration, as Taiko use different validation mechanisms compared to standard Ethereum networks.`,
		Args: cobra.NoArgs,
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if err := validateCustomNetwork(&flags.CustomFlags, network); err != nil {
				return err
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			services := []string{execution, consensus, taiko, tExecution}
			return runGenCmd(cmd.OutOrStdout(), &flags, sedgeAction, services)
		},
	}
	// Bind flags
	cmd.Flags().StringVarP(&flags.consensusName, "consensus", "c", "", "Consensus engine client, e.g. teku, lodestar, prysm, lighthouse, Nimbus. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name")
	cmd.Flags().StringVar(&flags.taikoName, "taiko-image", "", "Taiko Driver client image.")
	cmd.Flags().StringVar(&flags.l2ExecutionName, "l2-execution", "", "Taiko Execution client to be used, taiko-nethermind or taiko-geth. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name.")
	cmd.Flags().StringVarP(&flags.executionName, "execution", "e", "", "Execution engine client, e.g. geth, nethermind, besu, erigon. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name")
	cmd.Flags().StringVarP(&flags.executionApiUrl, "execution-api-url", "", "", "Set execution api url. If Set, will omit the creation of execution and beacon nodes, and only create optimism nodes.")
	cmd.Flags().StringVarP(&flags.consensusApiUrl, "consensus-url", "", "", "Set consensus api url. If Set, will omit the creation of execution and beacon nodes, and only create optimism nodes.")
	cmd.Flags().BoolVar(&flags.latestVersion, "latest", false, "Use the latest version of clients. This sets the \"latest\" tag on the client's docker images. Latest version might not work.")
	...
	cmd.Flags().StringSliceVar(&flags.fallbackEL, "fallback-execution-urls", []string{}, "Fallback/backup execution endpoints for the consensus client. Not supported by Teku. Example: 'sedge generate full-node -r --fallback-execution=https://mainnet.infura.io/v3/YOUR-PROJECT-ID,https://eth-mainnet.alchemyapi.io/v2/YOUR-PROJECT-ID'")
	cmd.Flags().StringArrayVar(&flags.elExtraFlags, "el-extra-flag", []string{}, "Additional flag to configure the execution client service in the generated docker-compose script. Example: 'sedge generate full-node --el-extra-flag \"<flag1>=value1\" --el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.ell2ExtraFlags, "el-l2-extra-flag", []string{}, "Additional flag to configure the execution client for taiko service in the generated docker-compose script. Example: 'sedge generate taiko-full-node --taiko-el-extra-flag \"<flag1>=value1\" --taiko-el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.taikoExtraFlags, "taiko-extra-flag", []string{}, "Additional flag to configure the taiko client service in the generated docker-compose script. Example: 'sedge generate taiko-full-node --taiko-extra-flag \"<flag1>=value1\" --taiko-	extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.clExtraFlags, "cl-extra-flag", []string{}, "Additional flag to configure the consensus client service in the generated docker-compose script. Example: 'sedge generate full-node --cl-extra-flag \"<flag1>=value1\" --cl-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringSliceVar(&flags.customEnodes, "execution-bootnodes", []string{}, "List of comma separated enodes to use as custom network peers for execution client.")
	cmd.Flags().StringSliceVar(&flags.customEnrs, "consensus-bootnodes", []string{}, "List of comma separated enrs to use as custom network peers for consensus client.")
...
func SurgeFullNodeSubCmd(sedgeAction actions.SedgeActions) *cobra.Command {
	var flags GenCmdFlags
	cmd := &cobra.Command{
		Use:   "surge-full-node [flags]",
		Short: "Generate a full node config for Surge",
		Long: `Generate a docker-compose and an environment file with a full node configuration for Surge networks.

This command sets up a Surge full node, which includes an execution client, a consensus client, a Surge driver client, and an Surge Execution node.

If you don't provide images for your clients, they will be chosen randomly. You can specify custom images for the Surge Driver and other nodes.

The command allows you to use external execution and consensus APIs instead of running your own nodes, by providing the respective URLs.

Additionally, you can use the syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client, for example 'sedge generate surge-full-node --execution nethermind:custom.image'. If you want to use the default docker image, just use the client name.

This command does not generate a validator configuration, as Surge use different validation mechanisms compared to standard Ethereum networks.`,
		Args: cobra.NoArgs,
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if err := validateCustomNetwork(&flags.CustomFlags, network); err != nil {
				return err
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			services := []string{execution, consensus, surge, sExecution}
			return runGenCmd(cmd.OutOrStdout(), &flags, sedgeAction, services)
		},
	}
	// Bind flags
	cmd.Flags().StringVarP(&flags.consensusName, "consensus", "c", "", "Consensus engine client, e.g. teku, lodestar, prysm, lighthouse, Nimbus. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name")
	cmd.Flags().StringVar(&flags.surgeName, "surge-image", "", "Surge Driver client image.")
	cmd.Flags().StringVar(&flags.l2ExecutionName, "l2-execution", "", "Surge Execution client to be used, surge-nethermind or surge-geth. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name.")
	cmd.Flags().StringVarP(&flags.executionName, "execution", "e", "", "Execution engine client, e.g. geth, nethermind, besu, erigon. Additionally, you can use this syntax '<CLIENT>:<DOCKER_IMAGE>' to override the docker image used for the client. If you want to use the default docker image, just use the client name")
	cmd.Flags().StringVarP(&flags.executionApiUrl, "execution-api-url", "", "", "Set execution api url. If Set, will omit the creation of execution and beacon nodes, and only create optimism nodes.")
	cmd.Flags().StringVarP(&flags.consensusApiUrl, "consensus-url", "", "", "Set consensus api url. If Set, will omit the creation of execution and beacon nodes, and only create optimism nodes.")
	cmd.Flags().BoolVar(&flags.latestVersion, "latest", false, "Use the latest version of clients. This sets the \"latest\" tag on the client's docker images. Latest version might not work.")
	...
	cmd.Flags().StringSliceVar(&flags.fallbackEL, "fallback-execution-urls", []string{}, "Fallback/backup execution endpoints for the consensus client. Not supported by Teku. Example: 'sedge generate full-node -r --fallback-execution=https://mainnet.infura.io/v3/YOUR-PROJECT-ID,https://eth-mainnet.alchemyapi.io/v2/YOUR-PROJECT-ID'")
	cmd.Flags().StringArrayVar(&flags.elExtraFlags, "el-extra-flag", []string{}, "Additional flag to configure the execution client service in the generated docker-compose script. Example: 'sedge generate full-node --el-extra-flag \"<flag1>=value1\" --el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.ell2ExtraFlags, "el-l2-extra-flag", []string{}, "Additional flag to configure the execution client for surge service in the generated docker-compose script. Example: 'sedge generate surge-full-node --surge-el-extra-flag \"<flag1>=value1\" --surge-el-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.surgeExtraFlags, "surge-extra-flag", []string{}, "Additional flag to configure the surge client service in the generated docker-compose script. Example: 'sedge generate surge-full-node --surge-extra-flag \"<flag1>=value1\" --surge-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringArrayVar(&flags.clExtraFlags, "cl-extra-flag", []string{}, "Additional flag to configure the consensus client service in the generated docker-compose script. Example: 'sedge generate full-node --cl-extra-flag \"<flag1>=value1\" --cl-extra-flag \"<flag2>=\\\"value2\\\"\"'")
	cmd.Flags().StringSliceVar(&flags.customEnodes, "execution-bootnodes", []string{}, "List of comma separated enodes to use as custom network peers for execution client.")
	cmd.Flags().StringSliceVar(&flags.customEnrs, "consensus-bootnodes", []string{}, "List of comma separated enrs to use as custom network peers for consensus client.")
...
func validatePOANetwork(network string) error {
	...
	found := false
	var networks = []string{"volta", "energyweb", "joc-mainnet", "joc-testnet", "linea-mainnet", "linea-sepolia", "xdc"}
