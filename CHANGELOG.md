...
- Fixed validator restart failure.
  ...
  - Validator waits for the consensus sync to finish: verifying that the `/eth/v1/node/health`
    consensus endpoint returns with the HTTP code 200 in its docker-compose `healthcheck`
    condition so that the validator waits for the consensus be healthy.
