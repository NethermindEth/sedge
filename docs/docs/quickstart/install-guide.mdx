...
Sedge is currently only available for Linux, macOS and Windows, and both amd64 and arm64 architectures. You can use any of the following methods to install Sedge:
...
First install the Go programming language following the [official instructions](https://go.dev/doc/install). You need at least the `1.18` version.
...
If you want to install Sedge in a Linux machine, we recommend you tu install some meta-packages to make the build process easier. You can install them with the following command in some of the major platforms:
