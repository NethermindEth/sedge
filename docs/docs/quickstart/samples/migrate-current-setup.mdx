...
3. Import keys, either by copying them to the new setup or using the one generated in the previous step.
...
2. You can either copy the keystore folder from your previous setup, or generate the keys again using the mnemonic

```
cp -r ./sedge-data/keystore ./new-path/keystore
```
