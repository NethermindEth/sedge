...
Notice that it says "Run the script with the selected services **execution, consensus**?". This is because by default only these nodes will be up and running at the beginning, as they need to sync with the network first and these can take days.
...
- Display running nodes
    ...
    Double check there are both `consensus-client` and `execution-client` entries. If one of them is missing then something [wrong has happened](/docs/troubleshooting). 
...
A **password prompt** may arise again because Sedge uses docker commands to run the staking-deposit-cli docker image, and these commands need to be executed with `sudo` by default. The required password is your **machine password**, not the keystore password.
...
Now you can proceed and finish the setup by pressing `Y`. This will run the validator service with it your validator client will be up and running. Make sure you created or prepared the keystore folder before this step. If you forgot to do it you can take your time, prepare the keystore folder and then press `Y`. This will also take you to the final logs, which describe how the validator is being run:
  ...
  2022-10-23 21:21:04 -- [INFO] Running command: docker compose -f docker-compose-scripts/docker-compose.yml ps --filter status=running
