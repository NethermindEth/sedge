...
# highlight-start
2022-07-31 21:06:40 -- [INFO] Created file sedge-data/docker-compose.yml
...
# highlight-next-line
2022-XX-XX 00:00:00 -- [INFO] Created file sedge-data/.env
```

### 4. Run the `docker-compose` script

Instructions on how to run the docker-compose script will be displayed and next to it a confirmation prompt.

```logs
2022-XX-XX 00:00:00 -- [INFO] Instructions for running docker-compose script

docker compose -f sedge-data/docker-compose.yml up -d execution consensus
...
- Run the docker-compose script
    ...
    ```
    2022-XX-XX 00:00:00 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml up -d execution consensus
    [+] Running 13/13
    ⠿ consensus Pulled
...
- Display running nodes
    ...
    ```
    2022-XX-XX 00:00:00 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml ps --filter status=running
...
- Track sync of execution and consensus node
    ...
    2022-XX-XX 00:00:00 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml ps --quiet execution
    2022-XX-XX 00:00:00 -- [INFO] Running command: docker inspect 13d0364c2ea3016c689def9b9e3e0f1d060ccca033274f3d30b8e2998a13a392

    2022-XX-XX 00:00:00 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml ps --quiet consensus
...
Date and time of the logs were modified to show more generic logs. Until this step, and depending on your internet connection, the entire process should take just a few minutes.
...
Users acknowledge that generating the keystore for other network apart from mainnet is an experimental and not audited feature. Nethermind takes no responsibility for any malfunctioning or lost money derived from an unexpected behavior during keystore generation.
...
docker run -it --rm -v /home/sedge/sedge-data/keystore/validator_keys:/app/validator_keys nethermindeth/staking-deposit-cli new-mnemonic --chain mainnet --keystore_password ********
...
# highlight-next-line
2022-XX-XX 00:00:00 -- [INFO] If everything went well, your keys can be found at: /home/sedge/sedge-data/keystore/validator_keys
...
When the execution and consensus clients are finally synced you will see the following logs:
  ...
  docker compose -f sedge-data/docker-compose.yml up -d validator

  # highlight-next-line
  ? Run validator service? [Y/n]
  ```

Now you can proceed and finish the setup pressing `Y`. This will run the validator service with it your validator client will be up and running. Make sure you created or prepared the keystore folder before this step. If you forgot to do it you can take your time, prepare the keystore folder and then press `Y`. This will also take you to the final logs, which describe you how to validator is being run:

  ```logs
  2022-XX-XX 00:00:00 -- [INFO] Checking if docker engine is on
  2022-XX-XX 00:00:00 -- [INFO] Running command: docker ps -a
  2022-XX-XX 00:00:00 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml up -d validator
  [+] Running 3/3
  ...
  2022-10-23 21:21:04 -- [INFO] Running command: docker compose -f sedge-data/docker-compose.yml ps --filter status=running
  2022-10-23 21:21:04 -- [DEBU] Running command with sudo.
  NAME                COMMAND                  SERVICE             STATUS              PORTS
  consensus-client    "lighthouse bn --dis…"   consensus           running             0.0.0.0:5054->5054/tcp, :::5054->5054/tcp, 4000/tcp, 0.0.0.0:9000->9000/tcp, :::9000->9000/tcp, 0.0.0.0:9000->9000/udp, :::9000->9000/udp
