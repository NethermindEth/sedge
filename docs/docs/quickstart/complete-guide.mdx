...
Users need to carefully select the network while using `sedge keys` for keystore generation. Users should use the `--network` flag if the target network is different from mainnet.
