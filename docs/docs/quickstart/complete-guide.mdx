...
If you want to use the latest version of the clients, and don't use the fixed image tested by us, you can generate the
setup using latest version of them using the following command: `sedge generate full-node --latest`. If you want to learn
more, you can check [here in our documentation](/docs/commands/generate).

You can use `--latest` flag with `full-node`, `execution`, `consensus` and `validator` commands.

If the clients release any version, you can just:
