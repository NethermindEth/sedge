...
main Ethereum network. The test network was established in March 2019 with Proof-of-Authority. It uses the Clique consensus
...
```
sedge cli --network goerli --no-mev-boost=true
```
...
```bash
sedge cli --no-validator --network=goerli
```
