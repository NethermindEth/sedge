...
Gnosis Chain is an EVM based Ethereum sidechain that is designed to be a platform for decentralized prediction markets.
It is a permissionless network that is governed by the Gnosis community.
...
```bash
sedge cli --no-validator --network=gnosis
```
